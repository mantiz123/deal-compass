...
export default function ContractSign() {
  ...
  const loadContract = async () => {
    try {
      ...
      if (!data.viewed_at) {
        ...
        await supabase
          .from('contracts')
          .update({ viewed_at: new Date().toISOString(), ip_address: ip, status: 'viewed' as any })
          .eq('id', data.id);
      }
  ...
  // Shared header
  const Header = () => (
    <div className="bg-[#0a0a14] border-b border-border/50 p-4 flex items-center justify-center gap-3">
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-md mx-auto mt-20 text-center p-6">
          ...
          <p className="text-muted-foreground">This signing link has expired or is no longer valid. Please contact Klose LLC for a new link.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-md mx-auto mt-20 text-center p-6">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">Already Signed</h2>
          <p className="text-muted-foreground">This document has already been signed. Thank you!</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-md mx-auto mt-20 text-center p-6">
          ...
          <p className="text-muted-foreground">{errorMsg || 'Something went wrong. Please try again or contact Klose LLC.'}</p>
        </div>
      </div>
    );
  }
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setAgreeBinding(v === true)}
                  />
                  <Label htmlFor="agree-binding" className="text-sm leading-tight cursor-pointer">
                ...
                <div className="flex items-start gap-2">
                  <Checkbox
                    id="agree-read"
                    checked={agreeRead}
                    onCheckedChange={(v) => setAgreeRead(v === true)}
                  />
                  <Label htmlFor="agree-read" className="text-sm leading-tight cursor-pointer">
                    I have read and understand all documents
                  </Label>
                </div>
              </div>

              <div className="flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => { setPageState('signing'); setSignatureData(null); }}>
                  Go Back
                </Button>
                <Button
                  className="flex-1"
                  disabled={!agreeBinding || !agreeRead || submitting}
          ...
          <CardContent className="space-y-2">
            ...
            {contract?.pdf_url && (
              <Button variant="outline" size="sm" onClick={() => window.open(contract.pdf_url, '_blank')}>
                <Eye className="h-4 w-4 mr-2" /> View Full Contract PDF
              </Button>
            )}
          </CardContent>
        </Card>
