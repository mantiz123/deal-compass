...
import { usePayments, useMarkPaymentPaid, Payment } from '@/hooks/usePayments';
import { format, isWithinInterval, parseISO } from 'date-fns';
import { es } from 'date-fns/locale';
import { cn } from '@/lib/utils';
import {
  MoreHorizontal, CheckCircle, Clock, XCircle, DollarSign,
...
const methodLabels: Record<string, string> = {
  check: 'Cheque', wire: 'Transferencia', zelle: 'Zelle',
  venmo: 'Venmo', cash: 'Efectivo', other: 'Otro',
};
    ...
    return payments.filter((payment) => {
      ...
      if (dateRange.from || dateRange.to) {
        const paymentDate = payment.payment_date
          ...
          : payment.created_at ? parseISO(payment.created_at) : null;
        if (paymentDate) {
          if (dateRange.from && dateRange.to) {
            if (!isWithinInterval(paymentDate, { start: dateRange.from, end: dateRange.to })) return false;
          } else if (dateRange.from && paymentDate < dateRange.from) return false;
          else if (dateRange.to && paymentDate > dateRange.to) return false;
        }
      }
      return true;
    });
              ...
              <TableRow key={payment.id}>
                ...
                <TableCell>
                  ...
                  {payment.reference_number && (
                    <span className="block text-xs text-muted-foreground">#{payment.reference_number}</span>
                  )}
                </TableCell>
                <TableCell>
