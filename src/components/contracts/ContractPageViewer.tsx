...
const SignatureBlock = ({ label, printLabel }: { label: string; printLabel: string }) => (
  <div className="mt-8 pt-4 border-t border-dashed border-border/50">
    <div className="w-64">
      ...
      <p className="text-xs text-muted-foreground mt-1">{printLabel}</p>
      <p className="text-xs text-muted-foreground mt-1">Date: ________________________</p>
    </div>
  </div>
);

const DualSignatureBlock = ({ leftLabel, rightLabel, leftPrint, rightPrint }: {
  leftLabel: string; rightLabel: string; leftPrint: string; rightPrint: string;
}) => (
...
// ─── AB CONTRACT PAGES ───
function ABPages({ d }: { d: Record<string, string> }) {
  const totalPages = 11;
  return (
    <>
      {/* Page 1 – Main Contract pt1 */}
      ...
      {/* Page 3 – Clauses 15-20 + Signatures */}
      <PageWrapper pageNum={3} total={totalPages}>
        ...
        <DualSignatureBlock leftLabel="Buyer Signature" rightLabel="Seller Signature" leftPrint="Klose LLC / Authorized Signatory" rightPrint="Seller Printed Name" />
      </PageWrapper>

      {/* Page 4-5 – Preliminary Seller Info Worksheet */}
      <PageWrapper pageNum={4} total={totalPages}>
        ...
        <div className="text-sm space-y-2">
          ...
          <p>Best Day/Time to Reach: ____________________</p>
        ...
        <div className="text-sm space-y-1">
          <p>Is this your primary residence? ☐ Yes ☐ No</p>
          <p>Are you the direct title holder? ☐ Yes ☐ No</p>
          <p>Are there any co-title holders? ☐ Yes ☐ No</p>
          <p>Name(s) of all title holders: ______________________________________________________</p>
          <p>How was the property acquired?</p>
          <ul className="list-disc ml-6 text-xs space-y-0.5">
            <li>Traditional Purchase (MLS/Realtor)</li>
            <li>Inherited / Gifted / Family Transfer</li>
            <li>Rent-to-Own Agreement</li>
            <li>Divorce Settlement</li>
            <li>Foreclosure Auction</li>
            <li>Tax Lien Sale</li>
            <li>Sheriff Sale</li>
            <li>Owner Financing</li>
            <li>Private Sale (Off-market)</li>
          </ul>
          <p>Years property has been owned: ________ years</p>
          <p>Are any of the title holders deceased? ☐ Yes ☐ No</p>
          <p>Has the property gone through probate? ☐ Yes ☐ No</p>
        </div>

        <h4 className="font-bold mt-4 mb-2">PROPERTY DETAILS</h4>
        <div className="text-sm space-y-1">
          <p>Property Type: ☐ Single Family ☐ Manufactured ☐ Multifamily ☐ Commercial/Mixed</p>
          <p>Occupancy Status: ☐ Vacant ☐ Owner Occupied ☐ Tenant Occupied</p>
        </div>
      </PageWrapper>
      ...
      <PageWrapper pageNum={5} total={totalPages}>
        <div className="text-sm space-y-2">
          ...
          <p>☐ {'<'}30 Days &nbsp; ☐ {'<'}60 Days &nbsp; ☐ {'<'}90 Days</p>
          <p className="font-bold mt-3">Is there a mortgage on the property? ☐ Yes ☐ No</p>
          <p>If yes, estimated balance: $ ______________________________</p>
          <p className="font-bold mt-3">Is the property in pre-foreclosure or pending foreclosure? ☐ Yes ☐ No</p>
          <p className="font-bold mt-3">Is there an HOA? ☐ Yes ☐ No</p>
          <p className="font-bold mt-3">RENTAL INFORMATION</p>
          <p>Is the property generating rental income? ☐ Yes ☐ No</p>
          <p>If yes, how much monthly? $ __________________</p>
          <p>Do you have a written lease agreement in place? ☐ Yes ☐ No</p>
          <p className="font-bold mt-3">ADDITIONAL INFORMATION</p>
          <p>Is there a Power of Attorney (POA) involved? ☐ Yes ☐ No</p>
          <p>Have you ever filed for bankruptcy? ☐ Yes ☐ No</p>
          <p>List any people you wish to consult before deciding to sell:</p>
          <p>________________________________________________________________________________</p>
        </div>
        <SignatureBlock label="Seller Signature" printLabel="Seller Printed Name" />
      </PageWrapper>
      ...
      {/* Page 8 – Notice of Non-Representation */}
      ...
      {/* Page 9 – Authorization to Sign (Special Limited POA) */}
      ...
      {/* Page 10 – Authorization for Release of Information */}
      <PageWrapper pageNum={10} total={totalPages}>
        ...
        <DualSignatureBlock leftLabel="Seller Signature" rightLabel="SSN (if required by title company)" leftPrint="Seller Printed Name" rightPrint="" />
      </PageWrapper>

      {/* Page 11 – Seller's Responsibility Acknowledgement */}
...
// ─── BC CONTRACT PAGES ───
function BCPages({ d }: { d: Record<string, string> }) {
  const totalPages = 6;
  ...
  return (
    <>
      ...
      <PageWrapper pageNum={1} total={totalPages}>
        ...
        <p className="text-sm leading-relaxed">ASSIGNEE represents and acknowledges receipt of the INITIAL PURCHASE AGREEMENT prior to the execution of this ASSIGNMENT. ASSIGNEE further agrees to accept all rights, obligations, and responsibilities the ASSIGNOR has agreed to as the BUYER in the INITIAL PURCHASE AGREEMENT. Furthermore, ASSIGNEE represents that they will: 1) Perform as required in good faith and within any time periods established; 2) Indemnify and hold harmless the ASSIGNOR from any claim or demand resulting from non-performance by the ASSIGNEE; 3) Indemnify and hold harmless the ASSIGNOR for any cost or expense in junk removal, cleaning, or otherwise removing items left by the previous owner; 4) Waive any and all right to further assign this ASSIGNMENT to a third-party; 5) Acknowledge and understand that the ASSIGNOR makes no warranty of any kind incident to this ASSIGNMENT.</p>
      </PageWrapper>
      ...
      <PageWrapper pageNum={2} total={totalPages}>
        ...
        <p className="text-sm leading-relaxed">Except as previously disclosed in writing to ASSIGNEE, ASSIGNOR has no knowledge of any underground tanks, faulty major appliances, faulty electrical, plumbing, heating, cooling, sewer, septic, well or water systems, structural or chimney defects, or hidden or latent defects (including leakage or water seepage) in the Property. EXCEPT: <V k="exceptions" data={d} fallback="None" />. ASSIGNEE acknowledges that they are purchasing the Property in its present physical condition ("as is" and "where is") as of the date of Acceptance, including any defects or problems specified in this Agreement or otherwise disclosed in writing by ASSIGNOR. ASSIGNOR shall have no obligation to remove any trash, debris, personal property, or other contents remaining on or in the property prior to closing. The responsibility for all removal and disposal of such items shall lie solely with the ASSIGNEE.</p>

        <h4 className="font-bold mt-4 mb-2">9. NON-REFUNDABLE OPTION FEE:</h4>
        <p className="text-sm leading-relaxed">ASSIGNEE has paid or will pay immediately upon execution of this ASSIGNMENT to <V k="title_company" data={d} /> (the "TITLE COMPANY") a non-refundable Assignment Option Fee of $ <V k="option_fee" data={d} /> by check or wire, to be held in a non-interest-bearing escrow account. Said Non-Refundable Option Fee shall be credited to the ASSIGNEE toward the total Assignment Contract Price. This ASSIGNMENT is non-binding until the Non-Refundable Option Fee is received by the TITLE COMPANY. ASSIGNEE has 24 hours to produce the Non-Refundable Option Fee; if not received, ASSIGNOR may continue marketing the property. The Non-Refundable Option Fee will be returned to ASSIGNEE only if ASSIGNOR cannot convey marketable title to the PROPERTY.</p>
        <div className="mt-2 text-sm">
          ...
          <p className="text-xs mt-1">(Parties may negotiate a different amount in writing.)</p>
        </div>

        <h4 className="font-bold mt-4 mb-2">10. CLOSING:</h4>
        <p className="text-sm leading-relaxed">ASSIGNOR will deliver this ASSIGNMENT to the TITLE COMPANY upon execution by both parties. Closing shall occur on or before <V k="closing_date" data={d} />, or within seven (7) days after objection to title has been cured.</p>

        <h4 className="font-bold mt-4 mb-2">11. TITLE POLICY:</h4>
        <p className="text-sm leading-relaxed">The SELLER shall furnish to ASSIGNEE at ASSIGNEE'S expense an Owner's Policy of Title Insurance issued by the TITLE COMPANY in the amount of the Sale Price, dated at or after closing, insuring ASSIGNEE against loss under the provisions of the Title Policy.</p>

        <h4 className="font-bold mt-4 mb-2">12. DEFAULT:</h4>
        <p className="text-sm leading-relaxed">If the ASSIGNOR is unable to perform, the ASSIGNEE'S express and sole remedy shall be limited to the termination of the ASSIGNMENT and return of an amount equal to the Non-Refundable Option Fee. If ASSIGNEE fails to comply herewith for any reason, ASSIGNOR may either enforce specific performance or terminate this ASSIGNMENT and retain the Non-Refundable Option Fee, thereby releasing all parties. If ASSIGNOR files suit for breach or enforcement, ASSIGNOR shall be entitled to recover all costs of such enforcement, including reasonable attorney's fees.</p>
      </PageWrapper>

      {/* Page 3 – Clauses 13-16 + Signatures */}
      <PageWrapper pageNum={3} total={totalPages}>
        <h4 className="font-bold mt-2 mb-2">13. MARKETABLE TITLE</h4>
        <p className="text-sm leading-relaxed">At the time of this Agreement, Seller owns a Contract for Sale & Purchase of the subject property. Buyer acknowledges that Seller currently does not own fee simple title to the subject property. This sale is contingent upon Seller obtaining marketable title. If Seller is unable to obtain marketable title, Seller shall return Buyer's option money, and this Agreement is void with no further obligations between parties.</p>

        <h4 className="font-bold mt-4 mb-2">14. HOLD HARMLESS AND ASSUMPTION OF LIABILITY</h4>
        <p className="text-sm leading-relaxed">In the event the Assignee has any damages or other liabilities caused by a third party, Assignor is to be held harmless by the Assignee for these damages or other liabilities. This specifically includes any third parties' duty to perform under a contract with the Assignor. Assignee understands and assumes the risk of loss for any liability caused by a third-party action.</p>
        ...
        <DualSignatureBlock leftLabel="Assignor Signature" rightLabel="Assignee Signature" leftPrint="Klose LLC / Authorized Signatory" rightPrint="Assignee Printed Name" />
      </PageWrapper>

      {/* Page 4 – Investor Disclosure (Buyer/Assignee version) */}
      <PageWrapper pageNum={4} total={totalPages}>
        <PageHeader title="INVESTOR DISCLOSURE STATEMENT" />
        <p className="text-center text-sm font-semibold mb-4">WORKING WITH KLOSE LLC</p>
        <p className="text-sm leading-relaxed mb-3">I, the undersigned, acknowledge and understand that Klose LLC ("Klose") is a for-profit real estate investment company organized under the laws of Wyoming and operating in multiple states including Alabama. Klose specializes in the acquisition and disposition of off-market real estate assets. Accordingly, the undersigned acknowledges the following:</p>
        ...
        <SignatureBlock label="Buyer/Assignee Signature" printLabel="Buyer/Assignee Printed Name" />
      </PageWrapper>

      {/* Page 5 – Notice of Non-Representation + Fair Housing */}
      <PageWrapper pageNum={5} total={totalPages}>
        <PageHeader title="NOTICES & DISCLOSURES" />
        <h4 className="font-bold mt-2 mb-2">NOTICE OF NON-REPRESENTATION</h4>
        <p className="text-sm leading-relaxed">You are hereby notified that Klose LLC and its members, managers, and employees do not represent you in any capacity as a real estate broker or agent. You should not assume that any representative of Klose LLC represents your interests unless you separately engage a licensed real estate agent or attorney. You are advised not to disclose any information you want held in confidence until you decide on representation. Your signature below acknowledges receipt of this notice and does not establish a brokerage relationship.</p>
        <SignatureBlock label="Buyer/Assignee Signature" printLabel="Buyer/Assignee Printed Name" />
        ...
        <h4 className="font-bold mt-4 mb-2">AFFILIATED BUSINESS DISCLOSURE</h4>
        <p className="text-sm leading-relaxed">Klose LLC and/or its affiliated companies and/or its management may have relationships with certain service providers, including title companies and lenders. Referrals to such providers may provide Klose LLC with a financial benefit. You are NOT required to use any specific title company, lender, or settlement service provider as a condition of your purchase or sale.</p>
      </PageWrapper>

      {/* Page 6 – Fair Housing signature */}
      <PageWrapper pageNum={6} total={totalPages}>
        <p className="text-sm leading-relaxed">By signing below, I/We acknowledge receipt of the Fair Housing Statement and the Affiliated Business Disclosure and understand that Klose LLC may refer us to certain service providers and may receive a financial or other benefit as a result.</p>
        <SignatureBlock label="Buyer/Assignee Signature" printLabel="Buyer/Assignee Printed Name" />
      </PageWrapper>
    </>
  );
...
// ─── AMENDMENT PAGES ───
function AmendmentPages({ d }: { d: Record<string, string> }) {
  const totalPages = 2;
  return (
    <>
      <PageWrapper pageNum={1} total={totalPages}>
        ...
        <DualSignatureBlock leftLabel="BUYER Signature" rightLabel="BUYER (2nd, if applicable)" leftPrint="Klose LLC / Authorized Signatory" rightPrint="Printed Name / Title" />
      </PageWrapper>

      <PageWrapper pageNum={2} total={totalPages}>
        <p className="text-sm mb-4">The party(ies) below have signed and acknowledge receipt of a copy.</p>
        <DualSignatureBlock leftLabel="SELLER Signature" rightLabel="SELLER (2nd, if applicable)" leftPrint="Seller Printed Name" rightPrint="Printed Name" />
      </PageWrapper>
    </>
  );
}

export default function ContractPageViewer({ contractType, data }: ContractPageViewerProps) {
  return (
    <div className="space-y-6">
      {contractType === 'AB' && <ABPages d={data} />}
      {contractType === 'BC' && <BCPages d={data} />}
      {contractType === 'AMENDMENT' && <AmendmentPages d={data} />}
    </div>
  );
}
